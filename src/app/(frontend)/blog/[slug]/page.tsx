          ...
          <div className={`flex flex-col md:flex-row items-stretch justify-center gap-8 md:gap-0 relative ${!prevPost || !nextPost ? 'md:max-w-xl mx-auto' : ''}`}>
            ...
            {prevPost && (
              <div className={`flex-1 flex flex-col ${nextPost ? 'border-b md:border-b-0 border-black/5 pb-8 md:pb-0 md:pr-12 lg:pr-24' : 'items-center text-center'}`}>
                ...
                >
                  <p className="text-[10px] tracking-[0.4em] uppercase text-black/30 mb-4 font-bold flex items-center gap-4 group-hover:text-[var(--accent)] transition-colors">
                    <ChevronLeft className="h-3 w-3" />
                    Попередня стаття
                  </p>
                  ...
                  <div className="h-[1px] w-0 group-hover:w-full bg-[var(--accent)] mt-3 transition-all duration-700 opacity-50" />
                </Link>
              </div>
            )}
